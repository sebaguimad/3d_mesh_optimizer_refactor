...
def describe_h(df: pd.DataFrame, col: str = "h_cbrtV", name: str = "") -> None:
    s = df[col]
    print(f"{name:<7} n={len(s):<8} min={s.min():.6g}  max={s.max():.6g}  mean={s.mean():.6g}  std={s.std():.6g}")

def plot_hist_single(df: pd.DataFrame, title: str, bins: np.ndarray, col: str = "h_cbrtV",
...
def plot_hist_overlay(coarse: pd.DataFrame, adapt: pd.DataFrame, title: str, bins: np.ndarray,
                      col: str = "h_cbrtV", save_path: Path | None = None, show: bool = True) -> None:
    import matplotlib.pyplot as plt
    plt.figure(figsize=(7, 5))
...
def main():
    ...
    if args.mode in ("separate", "both"):
        plot_hist_single(
            coarse, "Coarse: distribución de tamaños (3D)", bins, col=args.col,
            save_path=(save_dir / "hist_coarse.png") if save_dir else None,
            show=show
        )
        plot_hist_single(
            adapt,  "Adapt: distribución de tamaños (3D)", bins, col=args.col,
    ...
    if args.mode in ("overlay", "both"):
        plot_hist_overlay(
            coarse, adapt, "Coarse vs Adapt: distribución de tamaños (3D)", bins, col=args.col,
