...
def _export_mesh_inp_from_msh(*, gmsh_exe: str, msh_path: Path, out_inp: Path) -> None:
    ...
    # En algunos builds de Gmsh, convertir malla requiere -save/-0 explícito.
    candidate_cmds = [
        [gmsh_exe, str(msh_path), "-format", "inp", "-o", str(out_inp)],
        [gmsh_exe, str(msh_path), "-save", "-format", "inp", "-o", str(out_inp)],
        [gmsh_exe, str(msh_path), "-0", "-format", "inp", "-o", str(out_inp)],
    ]
    ...
    for cmd in candidate_cmds:
        ...
        if out_inp.exists():
            return
        print(f"⚠️  Gmsh terminó pero no creó {out_inp}. Reintentando con otra variante...")

    raise RuntimeError(
        "Gmsh terminó sin crear mesh.inp tras varios intentos. "
        f"Entrada: {msh_path} | salida esperada: {out_inp}. "
        "Prueba correr manualmente: "
        f"{gmsh_exe} {msh_path} -save -format inp -o {out_inp}"
    )
...
def _autogen_job_inp(
    *,
    ...
    lines = [
        # mesh.inp exportado por Gmsh suele traer *HEADING; evitamos duplicarlo.
...
def main() -> None:
    ...
    if args.backend == "fallback":
        ...
        if args.ccx_run:
            ...
            print(f"Info: mesh_inp exists = {mesh_inp.exists()} ({mesh_inp})")
            print(f"Info: job_inp exists = {inp.exists()} ({inp})")
